package com.anone.user.controller;
import com.alibaba.fastjson.JSON;
import com.anone.user.domain.User;
import com.anone.user.service.UserService;
...
public class UserController {
    ...
    public Result addUserPoint(Integer points) {
        //获取用户名
    ...
    public Result login(String username, String password, HttpServletResponse response) {
        //根据用户名查询用户密码===注意这里的username是主键
        User user = userService.findById(username);
        //对比查询出来的密码和输入密码是否匹配
        //参数说明：第一个是传入的参数密码，第二个是查出来的密码
        if (user!=null && BCrypt.checkpw(password,user.getPassword())) {
            //登录成功，就要将令牌返回给客户，客户再次访问需要持有令牌访问
            //设置令牌信息
            Map<String,String> jwtMap=new HashMap<String, String>();
            jwtMap.put("role","USER");
            jwtMap.put("success","SUCCESS");
            jwtMap.put("username",username);
            //创建令牌
            String jwt = JwtUtil.createJWT(UUID.randomUUID().toString(), JSON.toJSONString(jwtMap), 360000L);
            //将信息存入到cookie中,并响应到页面
            Cookie cookie=new Cookie("Authorization",jwt);
            response.addCookie(cookie);
            return new Result(true, StatusCode.OK, "成功登录",jwt);
        } else {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "User对象",value = "传入JSON数据",required = false) User user, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "String")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "User对象",value = "传入JSON数据",required = false) User user,@PathVariable("id")  String id){
        //设置主键值
        user.setUsername(id);
