...
import com.anone.user.domain.Areas;
import com.anone.user.service.AreasService;
...
public class AreasController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "Areas对象",value = "传入JSON数据",required = false) Areas areas, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "String")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Areas对象",value = "传入JSON数据",required = false) Areas areas,@PathVariable("id")  String id){
        //设置主键值
        areas.setAreaid(id);
