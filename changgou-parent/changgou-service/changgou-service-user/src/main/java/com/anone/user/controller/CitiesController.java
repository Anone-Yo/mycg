...
import com.anone.user.service.CitiesService;
...
public class CitiesController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "Cities对象",value = "传入JSON数据",required = false) Cities cities, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "String")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Cities对象",value = "传入JSON数据",required = false) Cities cities,@PathVariable("id")  String id){
        //设置主键值
        cities.setCityid(id);
