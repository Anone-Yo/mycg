...
import com.anone.user.domain.UndoLog;
import com.anone.user.service.UndoLogService;
...
public class UndoLogController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "UndoLog对象",value = "传入JSON数据",required = false) UndoLog undoLog, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "UndoLog对象",value = "传入JSON数据",required = false) UndoLog undoLog,@PathVariable("id")  Long id){
        //设置主键值
        undoLog.setId(id);
