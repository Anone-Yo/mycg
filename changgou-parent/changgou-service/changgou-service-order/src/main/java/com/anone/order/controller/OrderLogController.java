...
import com.anone.order.domain.OrderLog;
import com.anone.order.service.OrderLogService;
...
public class OrderLogController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "OrderLog对象",value = "传入JSON数据",required = false) OrderLog orderLog, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "String")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "OrderLog对象",value = "传入JSON数据",required = false) OrderLog orderLog,@PathVariable("id")  String id){
        //设置主键值
        orderLog.setId(id);
