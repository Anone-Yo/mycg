...
import com.anone.order.service.ReturnOrderItemService;
...
public class ReturnOrderItemController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "ReturnOrderItem对象",value = "传入JSON数据",required = false) ReturnOrderItem returnOrderItem, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "ReturnOrderItem对象",value = "传入JSON数据",required = false) ReturnOrderItem returnOrderItem,@PathVariable("id")  Long id){
        //设置主键值
