...
import com.anone.order.domain.ReturnCause;
import com.anone.order.service.ReturnCauseService;
...
public class ReturnCauseController {
    ...
    public Result<PageInfo> findPage(@RequestBody(required = false) @ApiParam(name = "ReturnCause对象",value = "传入JSON数据",required = false) ReturnCause returnCause, @PathVariable("page")  int page, @PathVariable("size")   int size){
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "ReturnCause对象",value = "传入JSON数据",required = false) ReturnCause returnCause,@PathVariable("id")  Integer id){
        //设置主键值
